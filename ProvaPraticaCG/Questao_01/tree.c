...
void initGL() {
    ...
    GLfloat spotDirection[] = { 0.0f, -1.0f, 0.0f };   // Direção do feixe (para baixo)

    adjustLightIntensity(); // Inicializa a intensidade da luz
    glLightfv(GL_LIGHT1, GL_POSITION, lightPos);        // Define a posição da luz
    glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, spotDirection); // Define a direção do spot
    glLightf(GL_LIGHT1, GL_SPOT_CUTOFF, 45.0f);         // Define o ângulo de cutoff (45 graus)
    glLightf(GL_LIGHT1, GL_SPOT_EXPONENT, 2.0f);        // Define o foco da luz (quanto menor, mais focada)
}
...
void adjustLightIntensity() {
    ...
    // Ajusta a intensidade da luz difusa e especular
...
void displayLightIntensity() {
    sprintf(windowTitle, "3D Tree | Light Intensity: %.2f", lightIntensity);
...
void display() {
    ...
    glutSwapBuffers();
    displayLightIntensity(); // Atualiza o FPS
}
